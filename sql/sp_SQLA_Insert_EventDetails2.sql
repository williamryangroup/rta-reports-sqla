...
BEGIN
	...
	select distinct e.PktNum, e.tOut, tOutHour = DATEPART(HOUR,e.tOut),
	 ...
	 where ((@StartDt = null) or (e.tOut >= @StartDt and ((@EndDt = null) or (@StartDt < @EndDt and e.tOut < @EndDt))))
	   and (@TruncateTable = 0 and not exists (select null from SQLA_EventDetails as d WITH (NOLOCK) where e.PktNum = d.PktNum and d.SourceTable = 'EVENT1'))
	   
	   and (@StartDt = null and @EndDt = null) and in EVENT4
