...
BEGIN
    ...
    SELECT EmpNum, EmpNameFirst, EmpNameLast, EmpJobType, c.PktNum, c.EventDisplay, d.Location,
		   ...
		   s.ShiftName, ShiftOrder = s.ShiftColumn, CustTier, d.tOutHour
...
BEGIN
	...
	select EmpNum, EmpNameFirst = ltrim(rtrim(emp.NameFirst)), EmpNameLast = ltrim(rtrim(emp.NameLast)), EmpJobType = ltrim(rtrim(emp.JobType)),
		   ...
		   EvtEsc = 0,
		   EvtAsn = 0,
		   EvtCmp = SUM(case when ActivityTypeID = 8 and Activity like '%Complete%' then 1 else 0 end),
		   AlertRcd = 0,
...
BEGIN
	...
	select EmpNum, 
	       ...
	       StatSecs = sum(case when ActivityEnd > @EndDt then datediff(second,ActivityStart,@EndDt) else datediff(second,ActivityStart,ActivityEnd) end)
	...
	BEGIN 
		...
		select distinct Time = f.tOut, 
		       State = f.State + (case when f.State = 'Complete' 
							           then (case when f.Source like '%.%.%.%' and f.Source <> @ServerIP then ' Mobile'
										          when f.Source like '%.%.%.%' and f.Source = @ServerIP then ' Workstation'
		       ...
		       [Source] = case when f.Source in ('RTSSPPE','RTSS.exe') then 'Server'
		                       ...
		                       when f.Source like '%.%.%.%' and f.Source <> @ServerIP then 'Device'
