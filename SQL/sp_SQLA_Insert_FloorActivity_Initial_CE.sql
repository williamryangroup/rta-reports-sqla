...
/****** Object:  StoredProcedure [dbo].[sp_SQLA_Insert_FloorActivity_Initial_CE]    Script Date: 06/15/2016 11:38:40 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_SQLA_Insert_FloorActivity_Initial_CE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [sp_SQLA_Insert_FloorActivity_Initial_CE]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_SQLA_Insert_FloorActivity_Initial_CE] 
...
BEGIN
	...
	select tOut, 2, 'Start', PktCbMsg, '', '', PktNum, '', EmpNumAuthorize, EmpNameAuthorize, DeviceIDRespond, '', '','EVENT1_CE',PktNum, '', '', ''
	  from RTSS.dbo.EVENT1_CE as e WITH (NOLOCK)
	...
	BEGIN
		...
		select distinct l.tEventState, 5, 
		 ...
		 inner join RTSS.dbo.EVENT1_CE as e WITH (NOLOCK)
	...
	BEGIN
		...
		select Time = e.tComplete, 9, State = 'Alert Resolved/Evt Cmp', Activity = ltrim(rtrim(alertType)), Location = ltrim(rtrim(a.location)), Zone, PktNum = EventTablePktNum, Tier = ltrim(rtrim(priority)), EmpNum = '', EmpName = '', [Source] = '', ID, '', 'ALERT1', ID, '', '', ''
		 ...
		 inner join dbo.EVENT1_CE as e WITH (NOLOCK)
		...
		select Time = a.tDismiss, 9, State = 'Alert Resolved', Activity = ltrim(rtrim(alertType)), Location = ltrim(rtrim(a.location)), l.Zone, PktNum = EventTablePktNum, Tier = ltrim(rtrim(priority)), EmpNum = '', EmpName = '', [Source] = '', ID, '', 'ALERT1', ID, '', '', ''
		  ...
		  left join dbo.EVENT1_CE as e WITH (NOLOCK)
		    on a.EventTablePktNum = e.PktNum
	...
	-- EVENT - Auto Reject - EventReject1/EVENT1_CE/EventStateLog1
	insert into SQLA_FloorActivity
	select tOut = er.tReject,
	 ...
	 inner join RTSS.dbo.EVENT1_CE as ev WITH (NOLOCK)
	 ...
	 where er.RejectReason like 'AUTOREJECT%' and ev.tReject is not null
	   and exists (select null from RTSS.dbo.EVENT_STATE_LOG1 as l2 where l2.PktNum = ev.PktNum and l2.EventTable = 'EVENT_CE')
	   and (@StartDt = null or tOut >= @StartDt)
	...
	-- EVENT - Auto Reject - EventReject/EVENT1_CE/EventStateLog1
	insert into SQLA_FloorActivity
	select tOut = er.tReject,
	       ActivityTypeID = 5,
	       State = 'Reject Auto',
	...
	select tOut = er.tOut,
	 ...
	 inner join (select RejPktNum = cast(left(right(rtrim([DESC]), LEN([DESC])-18), len(RIGHT(rtrim([DESC]),LEN([DESC])-18))-1) as int),
	  ...
	  left join RTSS.dbo.EVENT_STATE_LOG1 as l WITH (NOLOCK)
	   ...
	   and l.EventState = 'tDisplayMobile'
	 where exists (select null from RTSS.dbo.EVENT_STATE_LOG1 as l2 where l2.PktNum = ev.PktNum and l2.EventTable = 'EVENT_CE')
	   and (@StartDt = null or tOut >= @StartDt)
