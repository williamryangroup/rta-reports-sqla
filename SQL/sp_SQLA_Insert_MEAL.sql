...
IF EXISTS (select *
	         from dbo.sysobjects
	        where id = object_id(N'[dbo].[sp_SQLA_Insert_MEAL]')
...
BEGIN
	...
	DECLARE @UseAssetField char(255) = isnull((select case when Setting = 'Asset' then '1' else '0' end
		                                         from RTSS.dbo.SYSTEMSETTINGS WITH (NOLOCK)
												where ConfigSection = 'RTSSHH'
												  and ConfigParam = 'EventLocationOrAssetFieldName'),'0')

	insert into SQLA_MEAL (PktNum, Location, Zone, EmpNum, EmpName, EmpLicNum, tOut, EntryReason, ParentEventID,
		                   PktNumWitness1, PktNumSourceWitness1, EmpNumWitness1, EmpNameWitness1, EmpLicNumWitness1,
						   tWitness1, PktNumWitness2, PktNumSourceWitness2, EmpNumWitness2, EmpNameWitness2,
						   EmpLicNumWitness2, tWitness2, PktNumWitness3, PktNumSourceWitness3, EmpNumWitness3,
						   EmpNameWitness3, EmpLicNumWitness3, tWitness3, PktNumWitness4, PktNumSourceWitness4,
						   EmpNumWitness4, EmpNameWitness4, EmpLicNumWitness4, tWitness4, PktNumWitness5,
						   PktNumSourceWitness5, EmpNumWitness5, EmpNameWitness5, EmpLicNumWitness5, tWitness5,
						   EventComment, Asset, CardInEvtDisp, CardInEvtDesc, Source, tComplete)
	select ml.PktNum, ml.Location, ml.Zone,
	       EmpNum = case when ml.EmpNum is null or ml.EmpNum = ''
		                 then ISNULL(ISNULL(ISNULL(es.EmpNumComplete,et.EmpNumComplete),es.EmpNumAuthorize),et.EmpNumAuthorize)
						 else ml.EmpNum
						 end,
		   EmpName = case when ml.EmpName is null or ml.EmpName = ''
		                  then ISNULL(ISNULL(ISNULL(es.EmpNameComplete,et.EmpNameComplete),es.EmpNameAuthorize),et.EmpNameAuthorize)
						  else ml.EmpName
						  end,
		   ml.EmpLicNum,
		   ml.tOut,
		   ...
		   CardInEvtDesc = case when isnull(es.ResolutionDesc,et.ResolutionDesc) = 'Phantom'
		                        then ltrim(rtrim(isnull(isnull(isnull(isnull(crs2.EventDescription,crt2.EventDescription),rds2.CompleteReason),rdt2.CompleteReason),''))) + '; Phantom'
								else ltrim(rtrim(isnull(isnull(isnull(isnull(isnull(crs.EventDescription,crt.EventDescription),rds.CompleteReason),rdt.CompleteReason),es.ResolutionDesc),et.ResolutionDesc)))
								end,
	  ...
	  left join (select CmpReasonNum = cast(row_number() over(order by ConfigSection, ConfigParam)-1 as nvarchar), CompleteReason = Setting
	               from RTSS.dbo.SYSTEMSETTINGS with (nolock)
				  where ConfigSection = 'CompleteReason'
			    ) as rds
	    on rds.CmpReasonNum = es.ResolutionDesc
	  left join (select CmpReasonNum = cast(row_number() over(order by ConfigSection, ConfigParam)-1 as nvarchar), CompleteReason = Setting
	               from RTSS.dbo.SYSTEMSETTINGS with (nolock)
				  where ConfigSection = 'CompleteReason_ST'
			    ) as rdt
	    on rdt.CmpReasonNum = et.ResolutionDesc
	  left join RTSS.dbo.CARDIN_REASON as crs2 with (nolock)
	    on crs2.EventDisplay = left(es.EventComment,(case when CHARINDEX(';',es.EventComment) > 0
		                                                  then CHARINDEX(';',es.EventComment)-1
														  else 0
														  end))
	  left join RTSS.dbo.CARDIN_REASON_ST as crt2 with (nolock)
	    on crt2.EventDisplay = left(et.EventComment,(case when CHARINDEX(';',et.EventComment) > 0
		                                                  then CHARINDEX(';',et.EventComment)-1
														  else 0
														  end))
	  left join (select CmpReasonNum = cast(row_number() over(order by ConfigSection, ConfigParam)-1 as nvarchar), CompleteReason = Setting
	               from RTSS.dbo.SYSTEMSETTINGS with (nolock)
	    ...
	    on rds2.CmpReasonNum = left(es.EventComment,(case when CHARINDEX(';',es.EventComment) > 0
		                                                  then CHARINDEX(';',es.EventComment)-1
														  else 0
														  end))
	  left join (select CmpReasonNum = cast(row_number() over(order by ConfigSection, ConfigParam)-1 as nvarchar), CompleteReason = Setting
	    ...
	    on rdt2.CmpReasonNum = left(et.EventComment,(case when CHARINDEX(';',et.EventComment) > 0
		                                                  then CHARINDEX(';',et.EventComment)-1
														  else 0
														  end))
	 where not exists (select null
		                 from SQLA_MEAL as sm WITH (NOLOCK)
	                    where sm.PktNum = ml.PktNum)
