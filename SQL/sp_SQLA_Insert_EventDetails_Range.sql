...
/****** Object:  StoredProcedure [dbo].[sp_SQLA_Insert_EventDetails_Range]    Script Date: 06/15/2016 11:40:02 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_SQLA_Insert_EventDetails_Range]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [sp_SQLA_Insert_EventDetails_Range]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_SQLA_Insert_EventDetails_Range]
...
BEGIN
	...
	select distinct e.PktNum, e.tOut, tOutHour = DATEPART(HOUR,e.tOut),
	 ...
	 where (@StartDt = null or (e.tOut >= @StartDt and e.tOut < @EndDt))
	   and not exists (select null from SQLA_EventDetails as d WITH (NOLOCK) where e.PktNum = d.PktNum and d.SourceTable = 'EVENT1')
