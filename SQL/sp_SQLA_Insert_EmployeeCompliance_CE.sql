...
/****** Object:  StoredProcedure [dbo].[sp_SQLA_Insert_EmployeeCompliance_CE]    Script Date: 06/15/2016 11:38:17 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_SQLA_Insert_EmployeeCompliance_CE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [sp_SQLA_Insert_EmployeeCompliance_CE]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_SQLA_Insert_EmployeeCompliance_CE]
...
BEGIN
	...
	select e.PktNum from RTSS.dbo.EVENT4_CE as e WITH (NOLOCK)
	 where not exists 
		 ( select null from SQLA_EmployeeCompliance as c WITH (NOLOCK)
			where c.PktNum = e.PktNum
			  and c.SourceTable = 'EVENT1_CE' )
	...
	select l.EmpNum, l.PktNum, e.EventDisplay, CustTier = e.CustTierLevel, tOut = l.tEventState,
	 ...
	 inner join RTSS.dbo.EVENT4_CE as e WITH (NOLOCK)
